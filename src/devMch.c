...
int inst = mchSess->instance;
	...
	if ( MCH_INIT_DONE(stat) ) {
printf("init done; set to in progress\n");
mchStatSet( inst, MCH_MASK_INIT, MCH_MASK_INIT_IN_PROGRESS );
/*		if ( drvMchInitScan ) {
printf("before scaniorquest\n");
			scanIoRequest( drvMchInitScan );
printf("after scaniorquest\n");*
//}
	}
else
...
/* Change this to use mchGetSensorReadingStat */
			...
			if ( mchGetSensorReadingStat( mchData, data, sens ) ) //, sensor, lun, &responseSize) )
				s = ERROR;
			else {
...
/*			if ( !(s = mchMsgReadSensorWrapper( mchData, data, &mchSys->sens[sindex] )) ) {	
...
printf("%s init_fru_longout: FRU id is %i, index is %i\n",plongout->name, plongout->out.value.camacio.b, index);
