...
int dbg = MCH_DBG( mchStat[mchData->mchSess->instance] );
	...
	for ( i = 0; i < mchSys->fruCount; i++ ) {
		...
		if ( fru->siteType == PICMG_SITE_TYPE_FRONT_BOARD ) {
			mchSys->fruLkup[UTCA_FRU_TYPE_AMC_MIN + fru->siteNumber - 1] = i;
			if ( dbg >= MCH_DBG_MED ) {
				printf("assign_fru_lkup_atca: Found front board, "
				    "FRU index %i, lkup index %i\n", 
				    i,UTCA_FRU_TYPE_AMC_MIN + fru->siteNumber - 1); 
			}
		}
		else if ( fru->siteType == PICMG_SITE_TYPE_RTM ) {
			mchSys->fruLkup[UTCA_FRU_TYPE_RTM_MIN + fru->siteNumber - 1] = i;
			if ( dbg >= MCH_DBG_MED ) {
				printf("assign_fru_lkup_atca: Found RTM, FRU index %i, "
				    "lkup index %i\n", 
				    i, UTCA_FRU_TYPE_RTM_MIN + fru->siteNumber - 1);
			}
		}
		else if ( fru->sdr.entityId == ENTITY_ID_PICMG_SHMC ) {
			if ( (PICMG_FRU_TYPE_SHMC_MIN + shmCnt) > PICMG_FRU_TYPE_SHMC_MAX )
				continue;
			mchSys->fruLkup[PICMG_FRU_TYPE_SHMC_MAX - shmCnt] = i;
			if ( dbg >= MCH_DBG_MED ) {
				printf("assign_fru_lkup_atca: Found SHMC, FRU index %i, "
				    "lkup index %i\n", 
				    i, PICMG_FRU_TYPE_SHMC_MAX - shmCnt); 
			}
			shmCnt++;
		}
			...
			if ( (UTCA_FRU_TYPE_SHELF_MIN + shfCnt) > UTCA_FRU_TYPE_SHELF_MAX )
				continue;
			mchSys->fruLkup[UTCA_FRU_TYPE_SHELF_MIN + shfCnt] = i;
			if ( dbg >= MCH_DBG_MED ) {
				printf("assign_fru_lkup_atca: Found shelf fru info, "
				    "fru index %i, lkup index %i\n", 
				    i,UTCA_FRU_TYPE_SHELF_MIN + shfCnt); 
			}
			shfCnt++;
		}
		else if ( fru->sdr.entityId == ENTITY_ID_COOLING_UNIT) {
			mchSys->fruLkup[UTCA_FRU_TYPE_CU_MIN + cuCnt] = i;
			cuCnt++;
			if ( dbg >= MCH_DBG_MED ) {
				printf("assign_fru_lkup_atca: Found CU, FRU index %i, "
				    "lkup index %i\n", 
				    i, UTCA_FRU_TYPE_CU_MIN + cuCnt);
			}
		}
		else if ( fru->sdr.entityId == ENTITY_ID_POWER_MODULE ) {
			if ( (UTCA_FRU_TYPE_PM_MIN + pmCnt) > UTCA_FRU_TYPE_PM_MAX )
				continue;
			mchSys->fruLkup[UTCA_FRU_TYPE_PM_MIN + pmCnt] = i;
			pmCnt++;
			if ( dbg >= MCH_DBG_MED ) {
				printf("assign_fru_lkup_atca: Found PM, FRU index %i, "
				    "lkup index %i\n", 
				    i, UTCA_FRU_TYPE_PM_MIN + pmCnt);
			}
		}
	}
}
...
int dbg = MCH_DBG( mchStat[mchData->mchSess->instance] );
	...
	if ( -1 == (id = mchGetFruLkup( mchData, index )) ) /* Was not assigned FRU-lookup ID */
		return rval;
...
int i, natid;
	...
	if ( MCH_IS_NAT( mchData->mchSess->type ) && (sdr->sensType == SENSOR_TYPE_HOTSWAP) ) {
		if ( 2 != sscanf( (const char *)(sdr->str), "HS %d %s", &natid, buff ) )
			return;
		sens->fruId = natid; /* need to assign FRU index */

		for ( i = 0; i < mchSys->fruCount; i++ ) {
			if ( i == mchSys->fruLkup[sens->fruId] ) {
				sens->fruIndex = i;
				break;
			}
		}
	...
	for ( i = 0; i < mchSys->fruCount; i++ ) {
		...
		if ( (sens->sdr.entityId == mchSys->fru[i].sdr.entityId) && (sens->sdr.entityInst == mchSys->fru[i].sdr.entityInst ) ) {
			sens->fruId = mchSys->fru[i].sdr.fruId; /* FRU ID for associated FRU */
			sens->fruIndex = i; /* FRU ID for associated FRU */
			return;
		}
	}
	for ( i = 0; i < mchSys->mgmtCount; i++ ) {
